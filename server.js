...
db.exec(`
  CREATE TABLE IF NOT EXISTS properties (
    ...
    first_seen TEXT,
    last_seen TEXT,
    dead_reports INTEGER DEFAULT 0,
    last_dead_report TEXT
  )
`);

try { db.exec("ALTER TABLE properties ADD COLUMN dead_reports INTEGER DEFAULT 0"); } catch (e) {}
try { db.exec("ALTER TABLE properties ADD COLUMN last_dead_report TEXT"); } catch (e) {}
...
app.get("/stats", function (req, res) {
  ...
  const active = db.prepare(
    "SELECT COUNT(*) as count FROM properties WHERE last_seen >= datetime('now', '-90 days') AND (dead_reports < 3 OR dead_reports IS NULL)"
  ).get();
  const dead = db.prepare("SELECT COUNT(*) as count FROM properties WHERE dead_reports >= 3").get();
  const bySite = db.prepare("SELECT source_site, COUNT(*) as count FROM properties GROUP BY source_site").all();
  const recentCount = db.prepare(
    "SELECT COUNT(*) as count FROM properties WHERE last_seen >= datetime('now', '-7 days')"
  ).get();

  res.json({
    total_properties: total.count,
    active_properties: active.count,
    dead_properties: dead.count,
    by_site: bySite,
    active_last_7_days: recentCount.count
  });
});
...
app.post("/property", function (req, res) {
  ...
  if (existing) {
    db.prepare(
      "UPDATE properties SET last_seen = ?, bedrooms = ?, wanted_locations = ?, area = ?, postcode_area = ?, dead_reports = 0, last_dead_report = NULL WHERE fingerprint = ?"
    ).run(now, body.bedrooms || "", JSON.stringify(wanted), body.area || "", body.postcode_area || "", fp);
    return res.json({ status: "updated", fingerprint: fp });
  }

  db.prepare(`
    INSERT INTO properties
      (fingerprint, source_site, source_url, area, postcode_area, bedrooms, property_type, wanted_locations, first_seen, last_seen, dead_reports)
    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, 0)
...
app.post("/property/dead", function (req, res) {
  const body = req.body;
  if (!body.source_url) return res.status(400).json({ error: "source_url is required" });

  const fp = fingerprint(body.source_url);
  const now = new Date().toISOString();
  const existing = db.prepare("SELECT id, dead_reports FROM properties WHERE fingerprint = ?").get(fp);

  if (!existing) return res.json({ status: "not_found" });

  db.prepare(
    "UPDATE properties SET dead_reports = dead_reports + 1, last_dead_report = ? WHERE fingerprint = ?"
  ).run(now, fp);

  const updated = db.prepare("SELECT dead_reports FROM properties WHERE fingerprint = ?").get(fp);

  return res.json({
    status: "reported",
    dead_reports: updated.dead_reports,
    removed_from_chains: updated.dead_reports >= 3
  });
});
...
app.get("/chains", function (req, res) {
  ...
  const properties = db.prepare(
    "SELECT * FROM properties WHERE last_seen >= datetime('now', '-90 days') AND (dead_reports < 3 OR dead_reports IS NULL)"
  ).all();
